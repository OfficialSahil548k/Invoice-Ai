...
const StatusBadge = ({ status = "", size = "default", showIcon = true }) => {
  ...
  return (
    <div
      className={` inline-flex items-center ${sizeClasses[size]} rounded-full font-medium ${config.bg} ${config.text} border ${config.border} transition-all duration-300
    ease-out hover:scale-105 hover:shadow-sm group relative overflow-hidden`}
    >
      <div
        className={` absolute inset-0 bg-linear-to-r ${config.gradient} opacity-0 group-hover:opacity-5 transition-opacity-5 transition-opacity duration-300`}
      ></div>
...
export function StatusWithCount({ status, count, size = "default" }) {
  return (
    <div className=" inline-flex items-center gap-2 group">
      ...
      {count !== undefined && (
        <span className=" text-xs text-gray-500 font-medium bg-gray-100 px-2 py-2 rounded-full group-hover:bg-gray-200 transition-colors duration-200">
          {count}
        </span>
      )}
    </div>
